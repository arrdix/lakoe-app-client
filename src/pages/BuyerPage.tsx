...
function BuyerPage() {
    ...
    useEffect(() => {
        const fetchProduct = async () => {
            try {
                const response = await API.PRODUCT.GET_ONE_BY_ID(1) // Ubah ID produk sesuai kebutuhan
                if (response) {
    ...
    const handlePlusCount = () => {
        if (count < (product?.variant?.variantOption?.variantOptionValue?.stock ?? 0))
            setCount(count + 1)
    }
            ...
            <div className="flex px-14 gap-6 w-11/12 px-8 py-12 bg-white">
                ...
                <div className="w-full md:w-1/2 flex flex-col mt-6 md:mt-0">
                    ...
                    <p className="text-xl font-semibold text-gray-700 mb-4">
                        Rp {product.variant?.variantOption?.variantOptionValue?.price}
                    </p>
                    <div className="flex items-center mb-4">
                        ...
                        <span className="ml-4">
                            Stok {product.variant?.variantOption?.variantOptionValue?.stock}
                        </span>
                    </div>
                    {error && <div className="text-red-500">{error}</div>}

                    <div
                        className="mt-4 border p-2 overflow-auto"
